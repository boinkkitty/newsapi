	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectedErr != "" {
				...
				assert.Equal(t, tc.expectedStatusCode, storeErr.HTTPStatus())
			} else {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedNews: news.Record{
				...
				Content: "PostgreSQL 16 brings performance and monitoring improvements...",
				Source:  "postgresweekly.com",
				Tags:    []string{"postgresql", "release", "database"},
			},
		},
		{
			name:               "not found",
	...
	testCases := []struct {
	...
	}{
		{
			...
			expectedNews: []news.Record{
				...
				{
					Author:  "Bob Lee",
					Title:   "Understanding UUIDs in Databases",
					Summary: "Why UUIDs are useful for distributed systems.",
					Content: "UUIDs help prevent key collisions across systems, especially in microservices...",
					Source:  "dbtips.io",
					Tags:    []string{"uuid", "database", "design"},
				},
			},
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectedStatus != 0 {
				...
				var storeErr *news.CustomError
				assert.ErrorAs(t, err, &storeErr)
				assert.Equal(t, tc.expectedStatus, storeErr.HTTPStatus())
			} else {
				assert.NoError(t, err)
			}
		})
